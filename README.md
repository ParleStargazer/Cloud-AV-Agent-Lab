...
- Multi-run：第一版 MVP 已完成，复用现有 `single-run` 作为唯一单 case 原语，支持样本 manifest / selection、不可变 batch plan、preflight、串行真实 runner、resume / rerun、`multi_run_state.json`、`multi_run_events.jsonl` 和 aggregate summary。`runs/batch_20260530-235629_tencent-pc-manager` 已完成 94 样本云端串行 smoke：94/94 case completed，94/94 cleanup restored，94/94 evidence exported，最终 `completed_with_warnings`，0 个 environment failure。详细见 [MULTI_RUN_PLAN.md](docs/MULTI_RUN_PLAN.md)。
- `multi-run` 支持交互式引导：直接运行 `python -m cloud_av_agent_lab multi-run` 会提示填写产品、实例、快照、region、Guest Agent、Desktop Worker 和云端平台机样本目录。默认样本目录和 batch root 都解析到项目根 `runs`，即使从子目录启动也不会跑偏。云端平台机可用 `--platform-sample-dir runs\raw_sample` 作为 `--sample-dir runs\raw_sample --platform-sample-dir` 的简写；开发机仍应使用预生成 `--manifest`，不要扫描本地样本目录。下一步优化重点是 indexed 样本“测后即焚”、batch timing 聚合和 opt-in deferred cleanup。
