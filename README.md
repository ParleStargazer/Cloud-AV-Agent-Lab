...
- Multi-run：第一版 MVP 与三轮性能优化已完成，复用现有 `single-run` 作为唯一单 case 原语，支持样本 manifest / selection、不可变 batch plan、preflight、串行真实 runner、resume / rerun、`multi_run_state.json`、`multi_run_events.jsonl` 和 aggregate summary。当前实现已加入 timing aggregation、indexed 样本“测后即焚”、fastmode / deferred cleanup、upload 即时状态轮询、产品侧 observation probe、execution-stage probe、adaptive post-execution delay、runtime 参数记录和 Windows lock 写入鲁棒性收口。`runs/batch_20260601-162301_tencent-pc-manager` 已完成 94 样本封口 smoke：94/94 case completed，94/94 summary/evidence exported，94/94 indexed mirrors burned，最终 `completed_with_warnings`，`batch_cleanup_status = restored`。详细见 [MULTI_RUN_PLAN.md](docs/MULTI_RUN_PLAN.md)。
- `multi-run` 支持交互式引导：直接运行 `python -m cloud_av_agent_lab multi-run` 会提示填写产品、实例、快照、region、Guest Agent、Desktop Worker、固定延时、fastmode、probe 和 cleanup 策略。默认样本目录和 batch root 都解析到项目根 `runs`，即使从子目录启动也不会跑偏。云端平台机可用 `--platform-sample-dir runs\raw_sample` 作为 `--sample-dir runs\raw_sample --platform-sample-dir` 的简写；开发机仍应使用预生成 `--manifest`，不要扫描本地样本目录。后续优化重点转向 Guest Agent ready gate 的可解释性、restore/stop 耗时统计和 resume UX，而不是继续缩短 collector 本身。
