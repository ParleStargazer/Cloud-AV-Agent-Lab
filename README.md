...
- 产品扩展：下一个安全产品接入目标为“腾讯电脑管家”。按 `docs/PRODUCT_ONBOARDING.md` 接入新的 readiness probe、collector registry 条目、artifact policy 和产品归因规则，避免在 CLI、evaluator 或 exporter 中写死产品分支；等计划中的四种安全产品都适配完成后，再进入 `multi-run` / 报告聚合接口开发。
