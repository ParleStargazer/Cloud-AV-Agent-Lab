...
日志收集、简易评测和证据导出阶段已经形成 MVP：杀毒软件日志收集框架定义 collector 插件和统一证据 schema；collector 通过 registry 按 `product_id` 接入，新增产品需遵循 [PRODUCT_ONBOARDING.md](docs/PRODUCT_ONBOARDING.md) 的 readiness、collector、证据归因和 artifact 策略；Huorong、Windows Defender 和 Qihoo 360 已走通对应产品路径，其中 Windows Defender 已完成真实 `single-run` EICAR smoke test 封板，Qihoo 360 已完成 parser / readiness / collector / product binding MVP；Guest Agent 暴露日志收集接口、简易结论报告接口和证据包导出接口；本地 CLI 对应提供 `guest-collect-logs`、`guest-case-summary` 和 `guest-export-evidence`。
