...
日志收集、简易评测和证据导出阶段已经形成 MVP：杀毒软件日志收集框架定义 collector 插件和统一证据 schema；collector 通过 registry 按 `product_id` 接入，新增产品需遵循 [PRODUCT_ONBOARDING.md](docs/PRODUCT_ONBOARDING.md) 的 readiness、collector、证据归因和 artifact 策略；Huorong 和 Windows Defender 已走通两类产品路径，其中 Windows Defender 已完成真实 `single-run` EICAR smoke test 封板；Guest Agent 暴露日志收集接口、简易结论报告接口和证据包导出接口；本地 CLI 对应提供 `guest-collect-logs`、`guest-case-summary` 和 `guest-export-evidence`。下一候选产品计划为 360，预计实现路径会更接近 Huorong 的本地日志副本 + 只读解析模式，但仍需先完成日志位置、schema、归因字段和 raw artifact 边界审计。
