...
Cloud AV Agent Lab 是一个本地自动化编排框架，用于把 AI Agent、云端 Windows 虚拟机、杀软告警采集和结构化报告串成闭环。项目边界很明确：本地只保存代码、配置、任务状态和报告，不保存真实病毒样本，不下载真实病毒样本，不在本地执行任何样本。开发联调优先使用用户显式指定的 EICAR 或无害测试文件；进入真实样本验证时，样本只能在云端隔离主机或云端受控存储中流转，本地控制面仍不得执行、打开、解压、扫描或解析样本内容。

现有目录里的 `Spore` 可作为透明可控的 AI Agent 外壳，`vmware-mcp` 可作为本地 VMware 适配器参考。本项目新增的根目录框架优先面向云端虚拟机，后续也可以用同样接口接入 VMware。

## 安全边界

- 真实病毒样本只能在云端隔离环境或云端对象引用中出现，例如 `cos://bucket/redacted/case-001.bin`；本地控制面不保存、不下载、不执行、不解析真实样本。开发联调可用用户显式指定路径的 EICAR 或无害测试文件验证上传链路。
...
真实 single-run 现在会生成 `[guest_agent.desktop_worker]` 并默认要求云端 Control Agent 的 `/worker/status` 返回 ready 后才进入投送阶段。Desktop Worker 是为了解决 Windows Session 0 执行问题而引入的执行层：Control Agent 继续常驻 Session 0，真实 `execute_uploaded_sample` 会由 Control Agent 签发短期 execution lease 后转发给运行在交互式桌面 session 的 Worker；Worker 只执行当前 case 已登记样本，第一版支持 `.exe` 和受控 `.bat/.cmd`，并就近观测执行状态。需要诊断旧环境时可以临时使用 `--disable-desktop-worker-gate`，但这会回到不可判定的环境风险模型。详细见 [DESKTOP_WORKER.md](docs/DESKTOP_WORKER.md)。
...
python -m cloud_av_agent_lab guest-check-security-product-readiness --config configs/lab.local.toml --vm-id win10-tencent-manager --case-id case-001__tencent-pc-manager --product tencent-pc-manager
...
安全产品就绪检查新增最小闭环：`security_product_readiness` 与 `collectors` 并列，运行在投送前、collection 前，只做低侵入只读检查，不解析拦截日志、不读取样本内容、不启动/停止/修复安全产品。Huorong MVP 检查 `C:\ProgramData\Huorong\sysdiag` 与 `log.db` 是否存在，并把 `log.db` best-effort 复制到当前 case 的 `security-product-readiness\huorong\` 目录后只读检查副本 metadata。Windows Defender readiness 使用 Windows Event Log reader 抽象检查 Defender Operational channel 的日志可观察性；本地测试仍使用 fake reader，不读取真实 Event Log。Qihoo 360 readiness 检查 `360safe.Summary.dat` SQLite 日志可观察性，不判断 360 实时防护是否开启。Tencent PC Manager readiness 使用 `quarantine_metadata_observability` scope，只检查 QQPCMgr/TAV 隔离区 metadata 路径和 `TAVCacheFullEx.db` baseline。结果写入 `case_security_product_readiness.json`、`case_state.security_product_readiness`、`case_report.security_product_readiness` 和 `events.jsonl`。single-run 会在 `prepare-case` 后、`upload-sample` 前 warning-only 调用 readiness，所有状态都继续后续流程；evaluator 只在准备输出 `no_detection_observed` 时检查 readiness，`ready` 放行，其余状态转为 `inconclusive`。Evidence bundle 只纳入脱敏后的 `case_security_product_readiness.json`；`security-product-readiness\huorong\log.db*` 等 readiness snapshot 仍按 raw product log snapshot 排除。`ready` 只代表观察链路具备最低条件，`protection_state` 仍为 `unknown`，不证明实时防护已开启或一定会检出。CLI 为 `guest-check-security-product-readiness --product huorong|windows-defender|qihoo-360|tencent-pc-manager`，详细模型见 [SECURITY_PRODUCT_READINESS.md](docs/SECURITY_PRODUCT_READINESS.md)。

日志收集、简易评测和证据导出阶段已经形成 MVP：杀毒软件日志收集框架定义 collector 插件和统一证据 schema；collector 通过 registry 按 `product_id` 接入，新增产品需遵循 [PRODUCT_ONBOARDING.md](docs/PRODUCT_ONBOARDING.md) 的 readiness、collector、证据归因和 artifact 策略；Huorong、Windows Defender、Qihoo 360 和 Tencent PC Manager 已走通对应产品路径，其中 Windows Defender 已完成真实 `single-run` EICAR smoke test 封板，Qihoo 360 已完成 parser / readiness / collector / product binding，并通过云端隔离 `single-run` smoke 验证：管理员账号下的 Desktop Worker 能触发样本，360 写入 `Summary.dat`，collector 读取到 `av_quarantined` 证据并生成 `detected_or_blocked / high` 保守结论；Tencent PC Manager 已完成 TAV 隔离区 metadata parser/readiness/collector/product binding MVP，并通过云端隔离 `single-run` smoke 封板：collector 只观察 `<md5>` 隔离容器、`.ico` sidecar 和 `TAVCacheFullEx.db` metadata，不解密或收纳 raw TAV artifact，summary 可生成 `detected_or_blocked / high` 保守结论；Guest Agent 暴露日志收集接口、简易结论报告接口和证据包导出接口；本地 CLI 对应提供 `guest-collect-logs`、`guest-case-summary` 和 `guest-export-evidence`。

收集阶段新增 `guest-collect-logs --product huorong|windows-defender|qihoo-360|tencent-pc-manager`，通过云端 Guest Agent 读取已选择产品的日志并输出 `case_collection.json`。火绒 collector 会先把 `C:\ProgramData\Huorong\sysdiag\log.db`、`log.db-shm`、`log.db-wal` 复制到当前 case 的 `collection\huorong\` artifact 目录，再只读打开 SQLite 并自动发现最新的 `HrLogV3_*` 表。Windows Defender collector 通过 reader 抽象查询 Defender Operational channel，pywin32 缺失时返回结构化 `failed/unknown`，不是 500。Qihoo 360 collector 复制 `360safe.Summary.dat` 快照后只读解析 `FI/FQ`，并把 raw SQLite、union metadata、q3q 和样本本体排除在默认 redacted evidence bundle 之外；在 single-run 中，若受控执行已启动并观测到退出，编排层会先等待 45 秒再调用 collector，避免 360 弹窗默认处理和日志落库尚未完成时过早读取。Tencent PC Manager collector 只做 TAV 隔离 metadata 观察：当前 case MD5 对应的隔离容器可产生强/中置信归因，单独 `.ico` sidecar 或 TAV cache mtime 变化只能作为辅助活动信号。输出包含统一时间线、时间窗口、normalized product-log events 和保守 verdict：只有产品日志或产品 metadata 证据匹配当前 case 的 hash、sample path、root/child PID 或时间窗口时才判定 `intercepted`；单独的文件消失或进程不可观察不会被直接判定为杀软拦截。设计见 [COLLECTION_MODEL.md](docs/COLLECTION_MODEL.md)。
...
受控触发能力默认关闭。`guest-execute-sample` 默认请求 `dry_run_execute_uploaded_sample`，只校验当前 case 已登记上传样本的 metadata 和路径归属，不启动样本进程；显式加 `--real-action` 时会请求 `execute_uploaded_sample`，只有云端 Guest Agent 启用 execution、提供正确执行 token、Desktop Worker ready，且 Worker 校验短期单次 execution lease 后，才会启动当前 case 的已登记样本。真实启动发生在 Desktop Worker 内：`.exe` 直接以 `subprocess.Popen([sample_path], cwd=sample_dir, shell=False)` 启动，`.bat/.cmd` 使用固定 `C:\Windows\System32\cmd.exe /d /c call <sample_path>` argv 模板且仍为 `shell=False`；所有 handler 都使用 `DEVNULL` 标准流、`CREATE_NO_WINDOW`、`close_fds=True` 和最小化环境。不接受任意路径、命令、shell/cmd/PowerShell 或参数。执行观测是低侵入式只读元信息快照，只观测当前 case 的 root PID 及子进程，不缓存进程对象，不阻碍 Defender 或其他安全软件终止进程。开发联调仍建议使用 EICAR 或无害命令 exe/batch；真实样本验证必须限定在云端隔离环境中，本地控制面仍不执行、打开、解压、扫描或解析样本。proof 文件只作为早期联调辅助，长期评测需要结合投送状态、执行观测和只读安全产品日志证据。详细协议和单实例串行锁设计见 [GUEST_AGENT.md](docs/GUEST_AGENT.md)，受控触发模型见 [EXECUTION_MODEL.md](docs/EXECUTION_MODEL.md)，收集模型见 [COLLECTION_MODEL.md](docs/COLLECTION_MODEL.md)，Windows 免 Python 部署见 [GUEST_AGENT_DEPLOYMENT.md](docs/GUEST_AGENT_DEPLOYMENT.md)。
...
- Multi-run：第一版 MVP 已完成，复用现有 `single-run` 作为唯一单 case 原语，支持样本 manifest / selection、不可变 batch plan、preflight、串行真实 runner、resume / rerun、`multi_run_state.json`、`multi_run_events.jsonl` 和 aggregate summary。`runs/batch_20260530-235629_tencent-pc-manager` 已完成 94 样本云端串行 smoke：94/94 case completed，94/94 cleanup restored，94/94 evidence exported，最终 `completed_with_warnings`，0 个 environment failure。第一阶段性能优化也已封口：`runs/batch_20260531-185952_tencent-pc-manager` 完成 94 样本 fastmode smoke，batch cleanup restored，indexed mirror 全部测后即焚，timing 显示主要瓶颈已转向 upload status polling、Guest Agent ready、固定 collection delay 和 settling cooldown。详细见 [MULTI_RUN_PLAN.md](docs/MULTI_RUN_PLAN.md)。
- `multi-run` 支持交互式引导：直接运行 `python -m cloud_av_agent_lab multi-run` 会提示填写产品、实例、快照、region、Guest Agent、Desktop Worker、固定延时、fastmode 和 cleanup 策略。默认样本目录和 batch root 都解析到项目根 `runs`，即使从子目录启动也不会跑偏。云端平台机可用 `--platform-sample-dir runs\raw_sample` 作为 `--sample-dir runs\raw_sample --platform-sample-dir` 的简写；开发机仍应使用预生成 `--manifest`，不要扫描本地样本目录。下一步优化重点是 `upload-status-timeout-seconds` 参数化、fastmode 复用环境 quick ready gate、产品兼容 collector probe、adaptive post-execution delay 和 execution 阶段产品信号心跳。
